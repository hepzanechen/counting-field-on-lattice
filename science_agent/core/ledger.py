...
Status = Literal["INITIAL", "TESTING", "SUPPORTED", "FALSIFIED", "INCONCLUSIVE", "REFINED"]
...
class Ledger:
    ...
    def update_status(self, record_id: str, new_status: Status, evidence: dict | None = None):
    ...
    def add_revision(self, record_id: str, revision: dict):
        if record_id not in self.records:
            raise KeyError(f"Record {record_id} not found")
        rec = self.records[record_id]
