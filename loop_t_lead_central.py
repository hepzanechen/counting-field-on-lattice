...
results_dir = os.path.join("data", "ssh_chain", "vary_t_lead_central", f"results_{datetime.now().strftime('%Y%m%d_%H%M')}")
...
for t_lead_central_value in t_lead_central_values:
    print(f"\nProcessing t_lead_central = {t_lead_central_value}")
    ...
    filename = f"ssh_Nx{Nx}_t_lead_central{t_lead_central_value:.2f}.mat"
