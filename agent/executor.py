...
from utils.physics.invariants import (
...
def run_dual_path(H_BdG: torch.Tensor, make_leads, temperature: torch.Tensor,
                  E_batch: torch.Tensor, eta: float) -> dict:
    eta_t = torch.tensor(eta, dtype=torch.float32, device=E_batch.device)

    cf = calculation_cf_autograd(H_BdG, E_batch, eta_t, make_leads(),
...
def physics_judge(H_BdG: torch.Tensor, results: dict, eta: float,
                  results_eta_check: dict | None = None,
                  eta_check: float | None = None) -> list[dict]:
    report = [
    ...
    if not gf_conservation["passed"] and results_eta_check is not None:
