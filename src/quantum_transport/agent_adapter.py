...
from quantum_transport.hamiltonians.Central import KitaevChain, SSHChainBdG
...
class ModelSpec:
    ...
    def validate(self, proposed: dict[str, Any]) -> dict[str, float]:
...
def build_system(model_name: str, params: dict[str, Any], bias: float = 2.0,
...
class QuantumTransportDomain:
    ...
    def validate_parameters(
        ...
        if model_name not in CATALOG:
            raise ValueError(
                f"unknown model {model_name!r} (allowed: {list(CATALOG)})")
        return CATALOG[model_name].validate(proposed)

    def build_system(
        self, model_name: str, params: dict[str, Any], **options
    ):
        return build_system(model_name, params, **options)
